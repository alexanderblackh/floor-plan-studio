...
function renderAllMeasurements() {
  ...
  for (const p of state.placedFurniture) {
    ...
    if (distLeft > 3) {
      ...
      c += `<text x="${(lx1+lx2)/2}" y="${ly-3}" font-family="JetBrains Mono" font-size="${fontSize}" fill="${color}" text-anchor="middle">${Math.round(distLeft)}"</text>`;
    }
    ...
    if (distRight > 3) {
      ...
      c += `<text x="${(rx1+rx2)/2}" y="${ry-3}" font-family="JetBrains Mono" font-size="${fontSize}" fill="${color}" text-anchor="middle">${Math.round(distRight)}"</text>`;
    }
    ...
    if (distTop > 3) {
      ...
      c += `<text x="${tx+3}" y="${(ty1+ty2)/2}" font-family="JetBrains Mono" font-size="${fontSize}" fill="${color}" text-anchor="start">${Math.round(distTop)}"</text>`;
    }
    ...
    if (distBottom > 3) {
      ...
      c += `<text x="${bx+3}" y="${(by1+by2)/2}" font-family="JetBrains Mono" font-size="${fontSize}" fill="${color}" text-anchor="start">${Math.round(distBottom)}"</text>`;
    }
  }
...
export function renderAnchors() {
  ...
  for (const p of state.placedFurniture) {
    ...
    const pw = p.rotated ? d.h : d.w;
    const ph = p.rotated ? d.w : d.h;
    const cx1 = PAD + S(p.x + pw / 2);
    const cy1 = PAD + S(p.y + ph / 2);

    for (const targetId of p.anchors) {
      const target = state.placedFurniture.find(t => t.id === targetId);
      ...
      const cx2 = PAD + S(target.x + tw / 2);
      const cy2 = PAD + S(target.y + th / 2);

      // Calculate distance
      const dx = (target.x + tw/2) - (p.x + pw/2);
      const dy = (target.y + th/2) - (p.y + ph/2);
      const dist = Math.sqrt(dx*dx + dy*dy);

      // Draw anchor line
      c += `<line x1="${cx1}" y1="${cy1}" x2="${cx2}" y2="${cy2}" stroke="#9966cc66" stroke-width="1" stroke-dasharray="4 3"/>`;

      // Distance label
      const mx = (cx1+cx2)/2, my = (cy1+cy2)/2;
      c += `<rect x="${mx-15}" y="${my-7}" width="30" height="14" fill="#1a1a24cc" rx="3"/>`;
      c += `<text x="${mx}" y="${my+1}" font-family="JetBrains Mono" font-size="7" fill="#9966cc" text-anchor="middle" dominant-baseline="central">${Math.round(dist)}"</text>`;
    }
  }
